    ...
    def show(self):
        with ui.header().style(f'height: {self.height}px').classes('items-center justify-between p-0'):
            ...
            with ui.row().classes('gap-0 h-full'):
                ...
                ui.button(icon='fullscreen', on_click=self.maximize) \
                    .bind_visibility_from(self.window_state, 'max', lambda x: not x) \
                    .classes('h-full').props('flat color="white"')
                ui.button(icon='fullscreen_exit', on_click=self.restore) \
                    .bind_visibility_from(self.window_state, 'max') \
                    .classes('h-full').props('flat color="white"')
